...
Function Start-Logging {
    ...
    $Debugging = $False # Change to TRUE to output TEST logs.

    If ($Debugging -eq $True) {

        $Global:LogType = "TESTLOG"
    ...
    # Format: LOG_AFS_SHARE_DATE_TIME.txt
    $Global:Logfile = "$LogType" + "_AFS_" + "$strDate" + "-" + "$strTime" + ".txt"
    $Global:strLogfile = "$PSScriptRoot\Log\$Logfile"
}
...
Function Winfo { 
    ...
    $Date = UniversalDate
        ...
        Write-Output "$Date $Text" | Out-File $strLogfile -Append
    } 
    Else {
        Write-Host "$Date"
    }
...
#             if (!($CorrelationID -eq ""))  
#                {Write-Output "$(Get-Date -format G) - $CorrelationID - $message"}
#             else
#                {if ($global:CorrelationID -ne "" -and $global:CorrelationID -ne $null)  
#                    {Write-Output "$(Get-Date -format G) - $($global:CorrelationID) - $message"}}
            ...
            {write-host -foregroundcolor "gray" -backgroundcolor "Black"  "$(Get-Date -format G) - " -nonewline
              ...
              If($forcemaxchar -ne -1)
                     ...
                     Else
                           {if (!$NoMaxCharAlignment.ispresent) {$message = $message + (" " * ($forcemaxchar-$message.length) )}}}
              If($NoNewLine.IsPresent ) 
                     {Write-Host -foregroundcolor "Green" -backgroundcolor "Black" $message -NoNewline }
              Else
                     {Write-Host -foregroundcolor "Green" -backgroundcolor "Black" $message}
}
       
#==============================================================================
# Wverbose verbose message
#==============================================================================
function Wverbose
       {#Param(
       #      [Parameter(Mandatory=$false,Position=1)] [string]$message,    
...
write-host "Please do not use wverbose, use write-verbose directly instead"
exit 10

      }

#==============================================================================
# WWarn verbose message
#==============================================================================
function Wwarn
       {Param(
              [Parameter(Mandatory=$false,Position=1)] [string]$message,    
            ...
            {write-host -foregroundcolor "gray" -backgroundcolor "Black"  "$(Get-Date -format G) - " -nonewline
              ...
              If($forcemaxchar -ne -1)
                     ...
                     Else
                           {if (!$NoMaxCharAlignment.ispresent) {$message = $message + (" " * ($forcemaxchar-$message.length) )}}}
              If($NoNewLine.IsPresent) 
                     {Write-Host -foregroundcolor "yellow" -backgroundcolor "Black" $message -NoNewline }
              Else
            ...
            {write-host -foregroundcolor "gray" -backgroundcolor "Black"  "$(Get-Date -format G) - " -nonewline
              ...
              If($forcemaxchar -ne -1)
                     ...
                     Else
                           {if (!$NoMaxCharAlignment.ispresent) {$message = $message + (" " * ($forcemaxchar-$message.length) )}}}
              If($NoNewLine.IsPresent) 
                     {Write-Host -foregroundcolor "red" -backgroundcolor "Black" $message -NoNewline }
              Else
