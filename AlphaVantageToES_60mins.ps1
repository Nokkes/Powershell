...
try {
    Invoke-WebRequest -Uri "http://192.168.0.248:9200/stock-$($shardDate)/" -Body $($shardSettings) -Method PUT -ContentType "application/json" -Verbose
}
...
$symbols | % {
    $wr = Invoke-WebRequest -Uri "https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=$($_)&interval=60min&apikey=1PVZCB3K3UE8NKG2"
    ...
    $properties = $json.'Time Series (60min)' |
    ...
    foreach ($element in $properties) {
        ...
        foreach ($stock in $json.'Time Series (60min)'.$($element)) {
